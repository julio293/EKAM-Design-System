...
| C1.2 | `ekam-companion/styles.css:215` | HIS §3 #2 — `border-radius: 22px 22px 0 0` on the body scroll surface | Visual | The scroll surface tucks under the photo with a 22px round-corner — that's a soft container lifted from iOS. Drop to `0` and the photo's bottom edge becomes the divider. |
| C1.3 | `ekam-companion/styles.css:561-562` | S7 — heavy tray shadow | Critical | Replace with `border-top: 1px solid rgba(255,255,255,0.12)` on the forest tray. The tray already has a strong dark-on-cream contrast. |
| C1.4 | `ekam-companion/styles.css:778-780` | HIS §8 — `player-arrive` 320ms translateY(-4) animation | Behavioural (low) | Allowed (translate-up, no scale), but the −4px is fussy. Use `translateY(8px → 0)` to match `tray-arrive` and the system motion vocabulary. |
| C1.5 | `ekam-companion/styles.css:366, 401` | HIS §12 (44px tap targets) | Accessibility | `.audio__play` and `.reach__actions button` are 36–40px. Bump to 44px. |
